...
(function () {
    ...
    if (!chromep) {
        var chromep = window.chromep = {...chrome};
    }
    ...
    function promisify(f, parseCB) {
        return (...args) => {
            let safeArgs = args;
            let callback;
            // The chromep API functions all use arguments, so we can't use f.length to check
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    f(...safeArgs, (...cbArgs) => {
                        ...
                        // chromep extensions always fire the callback, but populate chromep.runtime.lastError with exception details
                        if (chromep.runtime.lastError) {
                            if (safeArgs[1] && safeArgs[1].action === "storeReady") {
                                resolve();
                            }
                            // Return as an error for the awaited catch block
                            reject(new Error(chromep.runtime.lastError.message || `Error thrown by API ${chromep.runtime.lastError}`));
                        } else {
                            if (parseCB) {
     ...
     * @param {object} api The chromep native API to extend
    ...
    // accessibilityFeatures https://developer.chromep.com/extensions/accessibilityFeatures
    const knownA11ySetting = ['get', 'set', 'clear'];

    // ContentSetting https://developer.chromep.com/extensions/contentSettings#type-ContentSetting
    const knownInContentSetting = ['clear', 'get', 'set', 'getResourceIdentifiers'];

    // StorageArea https://developer.chromep.com/extensions/storage#type-StorageArea
