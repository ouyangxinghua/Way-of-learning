...
(function () {
    ...
    chromep.tabs.executeAsyncFunction = async function (tab, action, ...params) {
...
Stack: ${error.stack}`)
    ...
    chromep.tabs.createAndWait = async function(createProperties, msTimeout) {
     ...
     * The result is an object containing the parameters passed to the callback for [chrome.tabs.onUpdated]{@link https://developer.chrome.com/extensions/tabs#event-onUpdated}.
     * Rejects if an error is encountered loading the tab, or if it times out. */
    chromep.tabs.reloadAndWait = async function(tabId, reloadProperties, msTimeout) {
