...
window.applyFootlockerCart = () => {
  ...
  if (
    ...
    // resolve(Math.max(0.00, price));
    window.localStorage.setItem("applyFootlockerCart", Math.max(0.0, price));
  }
  setTimeout(window.applyFootlockerCart, 100);
};

window.applyFinishlineCheckout = () => {
  ...
  if (
    ...
    if (priceDollar.indexOf("$") !== -1) {
      ...
      window.localStorage.setItem(
        "applyFinishlineCheckout",
        Math.max(0.0, price)
      );
    }
  }
  setTimeout(window.applyFinishlineCheckout, 101);
};
